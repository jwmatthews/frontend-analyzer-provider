...
Zig is used as the C cross-linker by `cargo-zigbuild`. **Version 0.14.x is required** — Zig 0.16+ has breaking changes in its `ar` implementation that cause build failures with C dependencies (ring, aws-lc-sys, etc.).
...
Proto generation is **disabled** during cross-compilation builds. Pre-generated `.rs` and `.bin` files in `crates/grpc/src/generated/` are used instead. This avoids needing `protoc` or network access during cross-compilation.

To regenerate protos (host-only):
...
```
dist/
  frontend-analyzer-provider-v0.0.3-x86_64-unknown-linux-gnu.tar.gz
  frontend-analyzer-provider-v0.0.3-aarch64-unknown-linux-musl.tar.gz
  frontend-analyzer-provider-v0.0.3-x86_64-pc-windows-gnu.zip
  frontend-analyzer-provider-v0.0.3-checksums.sha256
  ...
```
...
If you see a panic about missing generated proto files:

```bash
make generate-proto
```

This only needs to be done once. The generated files are committed to the repo.
